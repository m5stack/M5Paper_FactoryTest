    ...
    if (lifegame_cells_flag == 0) {
        for (int y = 1; y < CELL_Y - 1; y++) {
            for (int x = 1; x < CELL_X - 1; x++) {
                ...
                lifegame_canvas_0.fillRect((x - 1) * CELL_W, (y - 1) * CELL_H,
                                           CELL_W, CELL_H,
                                           lifegame_cells_1.readPixel(x, y));
            }
        }
        ...
        for (int y = 1; y < CELL_Y - 1; y++) {
            for (int x = 1; x < CELL_X - 1; x++) {
                ...
                lifegame_canvas_1.fillRect((x - 1) * CELL_W, (y - 1) * CELL_H,
                                           CELL_W, CELL_H,
                                           lifegame_cells_0.readPixel(x, y));
            }
        }
        lifegame_cells_flag = 0;
    }
