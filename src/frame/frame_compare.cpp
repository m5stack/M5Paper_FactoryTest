...
Frame_Compare::Frame_Compare(void) {
    ...
    for (int i = 1; i < 8; i++) {
        ...
        _key_updatemode[i]->AddArgs(EPDGUI_Button::EVENT_RELEASED, 0,
                                    _key_updatemode[i]);
        _key_updatemode[i]->AddArgs(EPDGUI_Button::EVENT_RELEASED, 1, _canvas);
        _key_updatemode[i]->AddArgs(EPDGUI_Button::EVENT_RELEASED, 2,
                                    _canvas_time);
        _key_updatemode[i]->Bind(EPDGUI_Button::EVENT_RELEASED,
                                 key_update_mode_cb);
    }
