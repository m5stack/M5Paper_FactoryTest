...
int Frame_txtReader::run() {
    ...
    if (_is_first) {
        ...
        if (cursor == 0) {
            ...
            uint32_t offset =
                renderText(_page_cursor[1], _render_len, _canvas_next);
            if (offset == 0) {
                _page_end     = 1;
                _end_accessed = true;
            } else {
                _page_cursor.insert(
                    std::pair<uint32_t, uint32_t>(2, cursor + offset));
            }
        }
        ...
        if (M5.BtnR.wasReleased() || (_key_operation == 1)) {
            ...
            if (_page != _page_end) {
                ...
                memcpy(_canvas_prev->frameBuffer(),
                       _canvas_current->frameBuffer(),
                       _canvas_current->getBufferSize());
                memcpy(_canvas_current->frameBuffer(),
                       _canvas_next->frameBuffer(),
                       _canvas_next->getBufferSize());

                if ((_end_accessed == false) || (_page != _page_end)) {
                    uint32_t offset = renderText(_page_cursor[_page + 1],
                                                 _render_len, _canvas_next);

                    if (offset != 0) {
                        if (_page_cursor.count(_page + 2) == 0) {
                            _page_cursor.insert(std::pair<uint32_t, uint32_t>(
                                _page + 2, _page_cursor[_page + 1] + offset));
                        }
            ...
            if (_page > 0) {
                ...
                memcpy(_canvas_next->frameBuffer(),
                       _canvas_current->frameBuffer(),
                       _canvas_current->getBufferSize());
                memcpy(_canvas_current->frameBuffer(),
                       _canvas_prev->frameBuffer(),
                       _canvas_prev->getBufferSize());
                if (_page != 0) {
                    renderText(_page_cursor[_page - 1], _render_len,
                               _canvas_prev);
                }
            }
        }
