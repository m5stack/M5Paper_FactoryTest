...
EPDGUI_Switch::EPDGUI_Switch(int16_t state_num, int16_t x, int16_t y, int16_t w,
                             int16_t h)
    : EPDGUI_Base(x, y, w, h) {
...
void EPDGUI_Switch::Bind(int16_t state,
    ...
    if (is_in_area) {
        ...
        if (_event == EVENT_PRESSED) {
            ...
            if (this->_func_cb_array[_state] != NULL) {
                this->_func_cb_array[_state](
                    this->_func_cb_param_array[_state]);
            }
        }
    }
}
