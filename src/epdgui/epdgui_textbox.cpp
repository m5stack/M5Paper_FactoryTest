...
EPDGUI_Textbox::EPDGUI_Textbox(int16_t x, int16_t y, int16_t w, int16_t h)
    : EPDGUI_Base(x, y, w, h) {
...
void EPDGUI_Textbox::SetTextMargin(int16_t left, int16_t top, int16_t right,
                                   int16_t bottom) {
    _margin_left   = left;
    _margin_top    = top;
...
void EPDGUI_Textbox::Draw(m5epd_update_mode_t mode) {
    ...
    if (_state == EVENT_NONE) {
        ...
        _canvas->drawRect(0, 0, _w, _h, 15);
        _canvas->setTextArea(_margin_left, _margin_top, _w - _margin_right,
                             _h - _margin_bottom);
        _canvas->print(_data);
...
void EPDGUI_Textbox::Bind(int16_t event,
