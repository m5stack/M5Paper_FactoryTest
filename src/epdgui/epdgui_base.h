...
class EPDGUI_Base {
   public:
    ...
    virtual void Bind(int16_t event,
                      void (*func_cb)(epdgui_args_vector_t&)) = 0;
    ...
    uint32_t* GetID_P(void) {
        return &_id;
    }
    void SetID(uint32_t id) {
        _id = id;
    }
    void SetCustomString(String s) {
        _user_custom_str = s;
    }
    String GetCustomString(void) {
        return _user_custom_str;
    }
    ...
    inline int16_t getRX() {
        return _rx;
    }
    inline int16_t getBY() {
        return _by;
    }
